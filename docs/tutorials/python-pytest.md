...
end-to-end (from the published `use-cases` tarball, running real `pytest`) by
[`tests/release/example-python-pytest.test.ts`](../../tests/release/example-python-pytest.test.ts).

> The only Node you install is the `use-cases` CLI itself (the trust engine). The code
...
```bash
npm i -g use-cases                  # provides the `use-cases` binary
...
```bash
use-cases matrix validate --repo . --json     # ok: true
```
...
```bash
use-cases scan --repo . --public-key public-key.pem --json
...
```bash
use-cases verify --repo . --all --out verification-results.jsonl \
...
use-cases prove --repo . --all --trusted-ci --append \
...
use-cases verify --repo . --all --out verification-results.jsonl --public-key public-key.pem --json
...
| Bound | `use-cases bind … --register-existing` | UNPROVEN |
| Verified (keyless, real pytest) | `use-cases verify --out …` | UNPROVEN (results only) |
| Proved (trusted, signed) | `use-cases prove --trusted-ci …` | **FRESH** |
| Production code broken | `use-cases verify` → `use-cases prove` | refused → not FRESH |
