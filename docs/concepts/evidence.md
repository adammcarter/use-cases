...
| Recorded by | `ucp evidence record` (agents, scripts, humans) | `ucp prove` in CI only |
...
ucp evidence record --repo . --use-case billing.core.apply_discount \
  --kind test_result --result pass --json

# Replay the append-only evidence history:
ucp evidence status --repo . --json

# Correct an earlier event without deleting history:
ucp evidence void --repo . --evidence <id> --expected-head <event> \
...
> Related but distinct: **showcase runs** (`ucp showcase …`) and **capsules**
> (`ucp capsule …`) record live demonstrations through their own ledger. Like
