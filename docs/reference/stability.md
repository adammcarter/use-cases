...
- **CLI command + subcommand names and their flags** — e.g. `use-cases matrix validate`, `use-cases verify`, `use-cases prove`, `use-cases bind`, `use-cases scan`. Renames/removals are breaking.
...
- MCP **resources** (`use-cases://…`) and **prompts** are exposed and read-only, but
