...
| Area | Status |
|---|---|
| **Identity** | Renamed to Use Cases Plugin — `@use-case-matrix/{core,cli,mcp}`, CLI `ucm`, MCP `ucm-mcp`, schema `$id`s on `use-case-matrix.dev` |
...
2. Configure a **Trusted Publisher** on each of the three packages (GitHub Actions, repo `adammcarter/use-case-matrix`, workflow `release.yml`).
