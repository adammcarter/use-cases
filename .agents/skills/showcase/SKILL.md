...
- Plan mode prepares only. Use `ucp plan showcase --json` or `ucp capsule plan --json`.
- Run mode records actual performed events. Use `ucp showcase start --json` only when the request clearly asks for a live run or the user agrees to perform one.
- A generated plan must not be described as a completed demo.

## Live Run Rules

- Record observations before verdicts with `ucp showcase record-observation --json`.
- Record verdicts with `ucp showcase record-verdict --json`.
- When a verdict fails or blocks, record a decision with `ucp showcase decide --json` before finishing.
- Pause or resume with `ucp showcase pause --json` and `ucp showcase resume --json`.
- Correct mistaken entries with `ucp showcase correct --json`; do not edit JSONL history by hand.
- Finish with `ucp showcase finish --json` and inspect state with `ucp showcase status --json`.
...
Agents may record agent observations and agent verdicts when allowed. Agents must not claim user approval, user sign-off, or user verification. User approval requires the designated trusted confirmation path, such as `ucp showcase approve --json` with an explicit user actor in a context where the user actually confirms.
