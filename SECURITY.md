...
- **Local users cannot mint FRESH.** The private signing key lives only in
  trusted CI. A developer without that key can run `verify` (no key required)
  but cannot produce a signed proof, so cannot mint a FRESH row locally.
...
## What UCM does NOT guarantee

We would rather under-promise than ship "append-only" marketing that isn't true.
These are real limits — read them before relying on UCM for a security decision.

- **It does not judge whether a verifier is semantically adequate.** UCM proves
...
- **A malicious maintainer can approve a weak policy.** Anyone who can change the
  matrix, verifier config, or release-gate policy can lower the bar. UCM records
...
- **A compromised CI runner or leaked signing key can mint proofs.** The signing
  ...
  CI hardening, branch protection, and key rotation — not in UCM alone.
