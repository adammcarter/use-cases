...
use-cases matrix validate --repo .            # matrix is clean
use-cases scan --repo .                       # row is UNBOUND until you register it

# register the already-marked span, then run the trust flow:
use-cases bind --row example.checkout.apply_coupon \
  --file src/coupon.py --register-existing
use-cases verify --all --out verify-results.jsonl --repo .        # runs pytest
...
use-cases prove --all --trusted-ci --signing-key-env UCM_CI_SIGNING_KEY \
  --verification-results verify-results.jsonl --append --repo .
use-cases scan --repo . --public-key ci-signing-key.pub.pem      # row reads FRESH
