...
export function migrateTestMatrix(options: {
  ...
  if (options.mode === "write" && !wouldWrite.some((operation) => operation.action === "conflict")) {
    ...
    writeFileSync(join(outPath, ".presentation-skills-migration.json"), `${JSON.stringify({
      schema_version: 1,
...
function renderDraft(featureSlug: string, rows: LegacyRow[], sourcePath: string, sourceDigest: string, outRel: string) {
  ...
  const document = {
    ...
    metadata: {
      lifecycle: "planned",
      extensions: {
        "use-cases-plugin.dev/migration": {
  ...
  const content = [
    "# Generated by presentation-skills migrate test-matrix.",
...
function plannedAction(path: string, content: string): "create" | "update_generated" | "skip_unchanged" | "conflict" {
  ...
  return current.includes("Generated by presentation-skills migrate test-matrix") ? "update_generated" : "conflict";
}
