// `use-cases recover` — drive a drifted / unproven row back to green in ONE command.
//
      ...
      const diagnostics = [
        {
          ...
          message:
            ...
            `Inspect the failure with \`use-cases verify --repo ${ctx.workspace_root} ${all ? "--all" : `--row ${rowId}`}\`.`,
          source_path: null,
          json_pointer: null,
    ...
    if (!recovered) {
      ...
      const hint = needsPublicKey
        ...
        : scanResult.exit_code !== 0
          ? `\`use-cases scan\` reported an integrity error (exit ${scanResult.exit_code}) — resolve it, then re-run \`use-cases recover\`.`
          : `Inspect the current state with \`use-cases scan --repo ${ctx.workspace_root}\`.`;
      return {
        envelope: createCliResult(
          "markers.recover",
          {
    ...
    return {
      envelope: createCliResult(
        "markers.recover",
        {
          ...
          // and .status exactly as they do from `use-cases scan`.
          status: scanResult.status
        },
        {
          ok: true,
