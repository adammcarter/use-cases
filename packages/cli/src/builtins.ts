...
// This is what lets `use-cases matrix list` work bare while `--json` stays byte-stable.
...
export function runBuiltinCli(argv: string[]): number {
  ...
  // Discoverability: `use-cases`, `use-cases --help`, `use-cases -h`, and `use-cases <command> --help`
...
function runHelp(argv: string[], options: { unknown?: boolean; json?: boolean } = {}): number {
  ...
  const unknownMessage = `No recognized command for '${requested ?? "(none)"}'. See the commands listed below or run \`use-cases --help\`.`;
  ...
  const data = {
    schema_version: 1,
    usage: "use-cases <command> [subcommand] [flags] --json",
    requested,
    commands
  };
...
function renderHelpText(
  ...
  lines.push("use-cases — the Use Cases CLI", "");
  lines.push("Usage: use-cases <command> [subcommand] [flags] [--json]", "");
  ...
  lines.push("");
  if (!detailed) {
    lines.push("Run `use-cases <command> --help` for that command's flags.");
  }
...
function runInit(argv: string[], wantsJson: boolean): number {
  ...
  if (wantsJson) {
    ...
    process.stderr.write(`${result.diagnostics[0]?.message ?? "use-cases init failed."}\n`);
  }
