        ...
        messages: [
          userMessage(
            [
              ...
              `- Bind a row to code:     ucm bind --repo ${repo} --row <row-id> --file <path> --mode explicit --start-line <n> --end-line <m> --json`,
              `                          (or --mode swift-func --line <n> for a Swift function body)`,
              `- Verify locally:         ucm verify --repo ${repo} --all --json`,
              ...
              `- Scan freshness:         ucm scan --repo ${repo} --json`,
              "                          each verified row reports local_status: VERIFIED_LOCAL (status stays UNPROVEN — that is a healthy keyless row). `ucm scan --gate` exits non-zero if a required row is below the bar (>= VERIFIED_LOCAL by default).",
              "",
              "OPT-IN UPGRADE — signed proofs (FRESH) for a release/audit gate, in trusted CI only:",
              `- Generate a keypair:     ucm keygen --out <dir-outside-repo> --ci github   (private key is a CI secret — never in the repo)`,
              `- Mint proofs (CI):       ucm prove --repo ${repo} --all --verification-results <results> --append --trusted-ci --signing-key-env <ENV> --json`,
              `- Gate the ledger:        ucm validate-ledger --repo ${repo} --json`,
              `- Release gate:           ucm scan --repo ${repo} --policy-mode release --gate --json`,
              "",
              "`prove` mints signed proofs and must run only in trusted CI; it is intentionally not exposed over MCP. Everyday agent work stays keyless. Use the read-only MCP resources (ucm://matrix, ucm://freshness, ucm://ledger, ucm://bindings) to inspect state at any point."
            ].join("\n")
          )
        ]
      };
        ...
        messages: [
          userMessage(
            [
              ...
              `   ucm bind --repo ${repo} --row ${row} --file ${file} --mode explicit --start-line <n> --end-line <m> --json`,
              "   For a Swift function body (span inferred from the function line):",
              `   ucm bind --repo ${repo} --row ${row} --file ${file} --mode swift-func --line <n> --json`,
              ...
              `   ucm verify --repo ${repo} --row ${row} --json`,
              "",
              ...
              "",
              "Binding only edits source + the append-only registry, and verify only writes an UNSIGNED local ledger. Neither mints a proof — signing to FRESH is the opt-in upgrade that runs in trusted CI (`ucm prove`), which is not available over MCP."
            ].join("\n")
          )
        ]
      };
        ...
        messages: [
          userMessage(
            [
              ...
              `   ucm scan --repo ${repo} --json   (read rows[].status, rows[].local_status and rows[].reasons for ${row})`,
              "",
              "2. If the binding moved (code edited/relocated), re-bind so the marker tracks the new span:",
              `   ucm bind --repo ${repo} --row ${row} --file <path> --mode explicit --start-line <n> --end-line <m> --json`,
              "",
              "3. Recover in one command — re-verify and report the new state:",
              `   ucm recover --repo ${repo} --row ${row} --json`,
              "   This re-runs the row's verifier (the same run as `ucm verify`), writes the unsigned results ledger, re-scans, and reports the resulting local_status/status. The row returns to VERIFIED_LOCAL. `ucm recover` NEVER fakes green: if the verifier genuinely fails it exits non-zero and names the failing row — fix the code or the test, then re-run.",
              `   To inspect a raw verifier run on its own, use \`ucm verify --repo ${repo} --row ${row} --json\`.`,
              "",
              "4. OPT-IN — also re-prove to signed FRESH (release/audit only, trusted CI):",
              `   ucm recover --repo ${repo} --row ${row} --signing-key-env UCM_CI_SIGNING_KEY --public-key <path> --json`,
              ...
              `   ucm scan --repo ${repo} --json   (expect local_status: VERIFIED_LOCAL, or status: FRESH if you re-proved)`,
              `   ucm validate-ledger --repo ${repo} --json   (signed tier only)`
            ].join("\n")
          )
        ]
      };
        ...
        messages: [
          userMessage(
            [
              ...
              `   ucm scan --repo ${repo} --policy-mode release --gate --json`,
