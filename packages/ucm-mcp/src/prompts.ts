...
// proofs — they explain the real `ucp` CLI workflow so an agent can drive it
// through the host. Every command named here is a real CLI command verified
// against packages/ucm-cli (matrix / bind / scan / verify / prove /
// validate-ledger / evidence). There is no `ucp init`; a workspace is adopted by
// authoring `use-cases-plugin.yml` + a `use-cases/` tree. `prove` is
        ...
        messages: [
          userMessage(
            [
              ...
              `- Validate the matrix:    ucp matrix validate --repo ${repo} --json`,
              `- List/inspect rows:      ucp matrix list --repo ${repo} --json`,
              `- Bind a row to code:     ucp bind --repo ${repo} --row <row-id> --file <path> --mode explicit --line <n> --json`,
              `                          (or --mode swift-func for Swift functions)`,
              `- Scan freshness:         ucp scan --repo ${repo} --policy-mode feature --json`,
              `- Verify a row locally:   ucp verify --repo ${repo} --row <row-id> --out results.jsonl --json`,
              "",
              "In CI (trusted runner only) mint and check proofs:",
              `- Mint proofs:            ucp prove --repo ${repo} --all --verification-results results.jsonl --append --trusted-ci --json`,
              `- Gate the ledger:        ucp validate-ledger --repo ${repo} --json`,
              `- Release gate:           ucp scan --repo ${repo} --policy-mode release --json`,
              "",
              "`prove` mints signed proofs and must run only in trusted CI; it is intentionally not exposed over MCP. Use the read-only MCP resources (ucp://matrix, ucp://freshness, ucp://ledger, ucp://bindings) to inspect state at any point."
            ].join("\n")
          )
        ]
      };
        ...
        messages: [
          userMessage(
            [
              ...
              `   ucp matrix list --repo ${repo} --json   (look for ${row})`,
              "",
              "2. Place a binding marker in the source and register it. For an explicit line/span:",
              `   ucp bind --repo ${repo} --row ${row} --file ${file} --mode explicit --line <n> --json`,
              "   For a Swift function body (span inferred from the function):",
              `   ucp bind --repo ${repo} --row ${row} --file ${file} --mode swift-func --json`,
              "   Add --suffix <name> to register more than one binding slug for the same row.",
              "",
              "3. Confirm the binding took and inspect freshness:",
              `   ucp scan --repo ${repo} --policy-mode feature --json`,
              `   The row should move from UNBOUND to UNPROVEN (a binding exists but no proof yet).`,
              "",
              "Binding only edits source + the append-only registry. It never mints a proof — proving happens later in trusted CI (`ucp prove`), which is not available over MCP."
            ].join("\n")
          )
        ]
      };
        ...
        messages: [
          userMessage(
            [
              ...
              `   ucp scan --repo ${repo} --policy-mode feature --json   (read rows[].status and rows[].reasons for ${row})`,
              "",
              "2. If the binding moved (code edited/relocated), re-bind so the marker tracks the new span:",
              `   ucp bind --repo ${repo} --row ${row} --file <path> --mode explicit --line <n> --json`,
              "",
              "3. Re-run verification locally to produce fresh results:",
              `   ucp verify --repo ${repo} --row ${row} --out results.jsonl --json`,
              "",
              "4. In trusted CI, mint a new proof from those results and append it to the ledger:",
              `   ucp prove --repo ${repo} --row ${row} --verification-results results.jsonl --append --trusted-ci --json`,
              ...
              `   ucp scan --repo ${repo} --policy-mode feature --json`,
              `   ucp validate-ledger --repo ${repo} --json`
            ].join("\n")
          )
        ]
      };
        ...
        messages: [
          userMessage(
            [
              ...
              `   ucp scan --repo ${repo} --policy-mode release --json`,
              ...
              `   ucp matrix status --repo ${repo} --json`,
              "",
              "3. Validate ledger/registry integrity (append-only discipline, signatures, hash chain):",
              `   ucp validate-ledger --repo ${repo} --json`,
              "",
              "If any required row is SUSPECT/UNPROVEN/UNBOUND/INVALID, recover it (see the ucp/recover-suspect-row prompt) before releasing.",
              "These are read-only checks — you can also inspect ucp://freshness, ucp://ledger, and ucp://matrix/status over MCP without running anything."
            ].join("\n")
          )
        ]
      };
