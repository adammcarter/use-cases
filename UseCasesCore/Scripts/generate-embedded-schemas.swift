...
// frozen schema files in `schemas/v1`.
//
...
//
// `EmbeddedSchemasTests` asserts the committed file still carries the exact
// bytes of every file on disk, so a schema edit without a regeneration fails
// the suite rather than shipping a stale copy (ADR 0007 decision 8).
//
...
// Sorted by name so the output is deterministic, and read from the directory so
// the frozen order in `SchemaRegistry` is never duplicated here: the test checks
// the key set against that list.
let fileNames = try FileManager.default
  .contentsOfDirectory(atPath: schemasDirectory.path)
  .filter { name in
...
FileHandle.standardError.write(
  Data("Wrote \(targetURL.path) (\(fileNames.count) schemas)\n".utf8),
)
