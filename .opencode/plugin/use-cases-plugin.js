// OpenCode plugin for the use-cases-plugin.
//
...
// (bootstrap/use-cases-plugin.md). See critical-info-bootstrap / delivery-shapes.
import { readFileSync } from "node:fs";
import { fileURLToPath } from "node:url";
import { dirname, resolve } from "node:path";

const here = dirname(fileURLToPath(import.meta.url));
// .opencode/plugin/use-cases-plugin.js -> repo root is two levels up.
const pluginRoot = resolve(here, "../..");

function readBootstrap() {
  try {
    return readFileSync(resolve(pluginRoot, "bootstrap/use-cases-plugin.md"), "utf8");
  } catch {
    return "Error reading use-cases-plugin bootstrap";
  }
}
