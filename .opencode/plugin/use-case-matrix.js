// OpenCode plugin for the use-case-matrix.
//
...
// (bootstrap/use-case-matrix.md). See critical-info-bootstrap / delivery-shapes.
import { readFileSync } from "node:fs";
import { fileURLToPath } from "node:url";
import { dirname, resolve } from "node:path";

const here = dirname(fileURLToPath(import.meta.url));
// .opencode/plugin/use-case-matrix.js -> repo root is two levels up.
const pluginRoot = resolve(here, "../..");

function readBootstrap() {
  try {
    return readFileSync(resolve(pluginRoot, "bootstrap/use-case-matrix.md"), "utf8");
  } catch {
    return "Error reading use-case-matrix bootstrap";
  }
}
