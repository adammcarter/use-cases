...
describe("P8 host profiles, projections, and conformance", () => {
  test("host profile schema rejects proof fields", () => {
    const badProfile = {
      ...
      projection_targets: [{ kind: "activation_stub", path: ".codex/use-cases.md", managed: true }],
    ...
    expect(
      validateBySchemaId("https://use-cases.dev/schemas/v1/host-profile.schema.json", badProfile)
  ...
  test("dry-run projection is deterministic and writes no files", () => {
    ...
    expect(first.operations).toEqual([
      expect.objectContaining({
        action: "create",
        path: ".claude/use-cases.md"
      }),
      expect.objectContaining({
        action: "create",
        path: ".use-cases-projection.json"
      })
    ]);
    expect(existsSync(join(workspaceRoot, ".claude", "use-cases.md"))).toBe(false);
  ...
  test("write projection is idempotent, manifest-backed, and thin", () => {
    ...
    const stub = readFileSync(join(workspaceRoot, ".claude", "use-cases.md"), "utf8");
    const manifest = JSON.parse(readFileSync(join(workspaceRoot, ".use-cases-projection.json"), "utf8"));
    ...
    expect(stub).toContain("use-cases:managed");
    expect(stub).toContain(".agents/skills");
    expect(stub).not.toContain("## Live Run Rules");
    expect(manifest).toMatchObject({
      host: "claude",
      surface: "claude.cli",
      generated_files: [expect.objectContaining({ path: ".claude/use-cases.md" })]
    });
  });
  ...
  test("write projection refuses a target whose parent dir symlinks outside the workspace", () => {
    ...
    // (.claude/use-cases.md) stays in-workspace as a string. But .claude is a
    ...
    const workspaceRoot = fixtureWorkspace();
    const outside = mkdtempSync(join(tmpdir(), "use-cases-hosts-outside-"));
    ...
    expect(existsSync(join(outside, "use-cases.md"))).toBe(false);
  });
...
function fixtureWorkspace(): string {
  const workspaceRoot = mkdtempSync(join(tmpdir(), "use-cases-hosts-"));
  ...
  rmSync(join(workspaceRoot, ".use-cases-projection.json"), { force: true });
