...
function readConfig(repoDir: string): Record<string, unknown> {
  const source = readFileSync(join(repoDir, "presentation-skills.yml"), "utf8");
...
describe("ucm init", () => {
  test("scaffolds a workspace that immediately passes matrix validate", () => {
    ...
    expect(created).toContain("presentation-skills.yml");
  ...
  test("refuses an existing workspace without --force (stable error, non-zero, no clobber)", () => {
    ...
    const firstConfig = readFileSync(join(repo, "presentation-skills.yml"), "utf8");
    ...
    expect(readFileSync(join(repo, "presentation-skills.yml"), "utf8")).toBe(firstConfig);
  });
