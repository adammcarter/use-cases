...
describe("P-sec MCP rejects unsafe run/item ids", () => {
  ...
  test.each(MALICIOUS_RUN_IDS)("showcase_status rejects run '%s' with UCP_INVALID_ID", (runId) => {
  ...
  test("showcase_record_observation rejects a traversal item with UCP_INVALID_ID", () => {
    const fixture = fixtureWorkspace("evidence-basic");
    const envelope = withMcpWriteMode(() =>
      callTool("showcase_record_observation", {
        repo: fixture,
...
// with the stable UCP_PATH_ESCAPE envelope and read NOTHING outside the workspace.
describe("P-sec MCP bounds plan_file to the workspace", () => {
  ...
  test("showcase_start rejects a traversal plan_file with UCP_PATH_ESCAPE", () => {
    const fixture = fixtureWorkspace("evidence-basic");
    const envelope = withMcpWriteMode(() =>
