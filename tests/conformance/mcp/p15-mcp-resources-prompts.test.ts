...
// The repo-scoped resources, by URI. use-cases://schemas and use-cases://config either need
...
describe("P15 MCP resources + prompts surface", () => {
  ...
  test("resources/list returns the expected workspace-state URIs with metadata", () => {
    ...
    expect(uris.some((uri) => uri.startsWith("use-cases://file") || uri.includes("{path}"))).toBe(false);
  });

  test("resources/read of use-cases://matrix returns valid structured matrix content", () => {
    const result = readResource(`use-cases://matrix?repo=${fixture}`);
    const contents = (result.result as { contents: Array<{ uri: string; mimeType: string; text: string }> }).contents;
    expect(contents).toHaveLength(1);
    expect(contents[0].mimeType).toBe("application/json");
    expect(contents[0].uri).toBe(`use-cases://matrix?repo=${fixture}`);
  ...
  test("resources/read of use-cases://schemas/{name} returns the requested public schema without needing a repo", () => {
    const result = readResource("use-cases://schemas/common.schema.json");
  ...
  test("resources/read of use-cases://schemas index lists available schema ids", () => {
    const result = readResource("use-cases://schemas");
    const contents = (result.result as { contents: Array<{ text: string }> }).contents;
  ...
  test("resources/read rejects a traversal repo path", () => {
    const result = readResource("use-cases://matrix?repo=../../../../../../../../etc");
  ...
  test("resources/read requires a repo when none is configured", () => {
    const result = readResource("use-cases://matrix");
    expect(result.error).toMatchObject({ code: expect.any(Number) });
  ...
  test("resources/read of an unknown resource returns a not-found error", () => {
    const result = readResource("use-cases://does-not-exist");
    expect(result.error).toMatchObject({ code: expect.any(Number) });
    expect(result.result).toBeUndefined();
  });
  ...
  test("prompts/list returns the expected guided-workflow prompt names", () => {
    ...
    for (const name of ["use-cases/adopt-repo", "use-cases/bind-row", "use-cases/recover-suspect-row", "use-cases/release-review"]) {
  ...
  test("prompts/get returns a grounded message and honours arguments", () => {
    const response = handleMcpMessage({
      ...
      id: 1,
      method: "prompts/get",
      params: { name: "use-cases/bind-row", arguments: { row: "auth.login" } }
    });
    ...
    expect(result.messages[0].content.text).toContain("use-cases bind");
    ...
    expect(result.messages[0].content.text).not.toContain("use-cases init");
  });
  ...
  test("prompts never expose prove on the MCP surface and keep proofs CI-mediated", () => {
    ...
    expect(names).not.toContain("use-cases/prove");

    const recover = handleMcpMessage({
      jsonrpc: "2.0",
      id: 2,
      method: "prompts/get",
      params: { name: "use-cases/recover-suspect-row", arguments: { row: "auth.login" } }
    });
  ...
  test("prompts/get for an unknown prompt returns an error", () => {
    const response = handleMcpMessage({ jsonrpc: "2.0", id: 1, method: "prompts/get", params: { name: "use-cases/nope" } });
