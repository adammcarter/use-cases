...
describe("P0 package entrypoints", () => {
  ...
  test("packed packages install into a clean consumer and expose imports and bins", () => {
    ...
    const tarballs = [
      "adammcarter-use-cases-core-0.4.0.tgz",
      "adammcarter-use-cases-cli-0.4.0.tgz",
      "adammcarter-use-cases-mcp-0.4.0.tgz"
    ...
    writeFileSync(
      ...
      [
        ...
        "if (info.name !== '@adammcarter/use-cases' || info.version !== '0.4.0') throw new Error('bad version export');"
      ].join("\n")
    );
    ...
    requireSuccess(cli);
    expect(JSON.parse(cli.stdout).data.version).toBe("0.4.0");
