...
describe("P10 TEST-MATRIX migration", () => {
  test("dry-run creates a reviewable report without laundering old proof", () => {
    ...
    expect(
      validateBySchemaId("https://use-cases.dev/schemas/v1/migration-test-matrix-result.schema.json", envelope.data)
  ...
  test("write mode writes only migrated use-case YAML and validates it", () => {
    ...
    expect(existsSync(join(fixture, "use-cases", "_migrated", ".use-cases-migration.json"))).toBe(true);
    expect(existsSync(join(fixture, "evidence"))).toBe(false);
...
function fixtureWorkspace(): string {
  const workspaceRoot = mkdtempSync(join(tmpdir(), "use-cases-migration-"));
